...
def build_sparse_adjacency_matrix(user_sessions, level,
                                  track_ids, data_split, **kwargs):
    track_art_map = None
    ...
    if level == 'sess':
        ...
        start_idx = 1 if n_last_sess > 0 else 0
        for uid, sessions in tqdm(user_sessions.items(),
                                  desc='Calculate sess-level co-occ freq...'):
        ...
        for uid, sessions in tqdm(user_sessions.items(),
            ...
            for sess in sessions[:split_idx]:
        ...
        for uid, hist in tqdm(user_hist.items(),
                              desc='Calculate user-level co-occ freq...'):
            id_list = list(hist)
