...
class Model:
    ...
    def __init__(self, sess, params, n_users, n_items, pretrained_embs):
        ...
        self.embedding_type = params.get('embedding_type', 'svd')
        self.embedding_trainable = params.get('embedding_trainable', True)
        ...
        self.initializer = pretrained_embs[f'{self.embedding_type}_embeddings']
        # parameters for Self-Attention Module
        self.input_scale = model_params.get('input_scale', False)
        if 'sab' in model_params:
    ...
    def _create_variables(self, reuse=None):
        ...
        if self.embedding_trainable is not True:
            self.logger.debug('--> FROZEN INPUT')
        # item embedding
    ...
    def _build_optimizer(self, lr, step):
        ...
        if self.optimizer == self.SGD:
            ...
            raise Au2ActrError(f'Unknown optimizer type {self.optimizer}')
    ...
    def _activation(cls, act_type='None'):
        if act_type == 'none':
            ...
            raise Au2ActrError(f'Not support activation of type {act_type}')
